    ...
    <div className="flex min-h-screen flex-col bg-black text-white">
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-16 sm:px-6 lg:px-8">
        <div className="w-full max-w-md space-y-8 rounded-2xl border border-zinc-800 bg-zinc-950 p-8 shadow-2xl">
          ...
          <div className="relative w-full h-12 overflow-hidden rounded-xl group cursor-pointer" onClick={() => {
            // Fallback alert if Google Script fails to load (e.g. no internet or no client ID)
            ...
            {/* Note: opacity is set to 2% (0.02) to bypass clickjacking protection, and scale-150 ensures full coverage */}
            <div className="absolute inset-0 z-10 w-full h-full opacity-[0.02] overflow-hidden">
              <div id="google-signin-btn" className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 scale-150 origin-center"></div>
            </div>
          </div>
        </div>
      </div>
